...
public class ProcessSegundaOutputPrueba {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		Process proc1 = new ProcessBuilder(PWS,"-c", COMMAND_HELP).redirectErrorStream(true).start();
		Process proc2 = new ProcessBuilder(PWS,"-c", COMMAND_TREE).redirectErrorStream(true).start();
		//Thread.sleep(6000);
		InputStream inputStream1 = proc1.getInputStream();//asociado a la salida standar del hijo
		InputStream inputStream2 = proc2.getInputStream();
			...
			Process proc3 = new ProcessBuilder(PWS,"-c", COMMAND_DIR).redirectErrorStream(true).start();
