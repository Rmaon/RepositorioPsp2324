...
public class EjerciciosEjemploProcessbuilder {
    
    private static final String EDGE = "/usr/bin/microsoft-edge";
	private static final String PWS = "/usr/bin/bash";
...
//		Process process4 = new ProcessBuilder(PWS,"-c", " dam2").redirectErrorStream(true).start();
//		
//		process3.waitFor();
//		Thread.sleep(3000);
//		process3.destroy();
...
//		process5 redirige su datos de error al archivo errores.txt
		Process process5 = new ProcessBuilder(PWS,"-c", COMMAND_HELP).redirectError(new File("errores5.txt")).start();
		Process process6 = new ProcessBuilder(PWS,"-c", " dam2").redirectError(new File("errores6.txt")).start();
