...
public class ProcessTercerPrincipalExecutor {
	...
	static File path = new File("/home/ramon/psp/ProcessTercerPrincipal/build/classes");
	private final static File FILE = new File("exportT.txt");
	private final static String ENTER = "\n";
	
	public static void main(String[] args) throws IOException, InterruptedException {
		
		Process processTr = new ProcessBuilder("java","processtercerprincipal.TransformarString")
				.redirectErrorStream(true).directory(path).start();	
		Process processEx = new ProcessBuilder("java","processtercerprincipal.ExportarAFile",FILE.toString())
				.redirectErrorStream(true).directory(path).start();
		//Los output indican que salen para poder ser recogidos el proceso hijo
